...
with tab3:
    ...
    if st.session_state.analysis_results.get("tab3"):
        ...
        st.subheader("📊 가격 분포")
        fig = px.histogram(
            df_valid, x="lprice", nbins=30,
            labels={"lprice": "가격 (원)", "count": "상품 수"},
            template="plotly_dark", color_discrete_sequence=["#3b82f6"]
        )
        fig.add_vline(x=df_valid["lprice"].median(), line_dash="dash", line_color="#4ade80", annotation_text=f"중앙값: {df_valid['lprice'].median():,.0f}원")
        fig.update_layout(
            paper_bgcolor='rgba(0,0,0,0)',
            plot_bgcolor='rgba(0,0,0,0)',
            font_family="Pretendard",
...
with tab4:
    ...
    # ===== 상품 가격 비교 =====
        ...
        if st.session_state.analysis_results.get("tab4_price"):
            ...
            # 호버 정보 한국어로 변경
            fig.update_traces(
                hovertemplate="<b>%{x}</b><br>최대: %{customdata[0]:,.0f}원<br>Q3: %{customdata[1]:,.0f}원<br>중앙: %{customdata[2]:,.0f}원<br>Q1: %{customdata[3]:,.0f}원<br>최소: %{customdata[4]:,.0f}원<extra></extra>"
            )
            # 각 브랜드별 통계 데이터 추가
            for i, trace in enumerate(fig.data):
                brand_name = trace.name
                brand_data = c_df[c_df["brand_query"] == brand_name]["lprice"]
                if not brand_data.empty:
                    trace.customdata = [[
                        brand_data.max(),
                        brand_data.quantile(0.75),
                        brand_data.median(),
                        brand_data.quantile(0.25),
                        brand_data.min()
                    ]]
            st.plotly_chart(fig, use_container_width=True)
            ...
            fig.update_traces(
                hovertemplate="<b>%{x}</b><br>평균가: %{y:,.0f}원<extra></extra>"
            )
            st.plotly_chart(fig, use_container_width=True)
